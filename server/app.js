...
// }
const envrs = dotenv.config({path:__dirname+"/env/dev.env"});
if (envrs.error) {
  throw envrs.error
}
...
app.use('/public', express.static(path.join(__dirname, '..', 'public'))); // 정적인 파일들을 제공 (public 폴더 내 css, js, 이미지 등 정적인 파일을 넣고 브라우저로 접근 가능)
...
app.use((req, res, next) => {
  ...
  // console.log('3', req.session.views);
